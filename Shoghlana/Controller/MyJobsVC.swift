...
class MyJobsVC: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        var jobId = jobsList?[indexPath.row].job_id ?? 0
        var jobIdString = ("\(jobId)")
        
        print("joooob id: ",jobIdString)
        UserDefaults.standard.setValue(jobIdString, forKey: "selectedJob")
        
        let vc = UIStoryboard.init(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "JobDetailsVC")as! JobDetailsVC
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            jobsList?.remove(at: indexPath.row)
    ...
    func getAppliedJobs(){
        ...
        JobsModelRequest().appliedJobs(controller: self, apiTitle: "Applied Jobs", view: self.view) { (jobsModel) in
